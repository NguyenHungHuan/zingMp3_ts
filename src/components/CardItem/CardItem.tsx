...
export default function CardItem({
  ...
  return (
    <div className={`${className} ${(active || stateIdSong === dataItem.encodeId) && customBgActive}`}>
      <div className={`flex ${hideAlbum ? 'w-full' : 'w-1/2'} items-center gap-[10px]`}>
        {number && (
          <div className='mr-2 flex items-center justify-center text-[12px] font-bold'>
            <span
        ...
        <div className='flex w-full flex-col gap-[3px] break-words font-medium'>
          ...
          {isDate && (
            <span className='line-clamp-1 cursor-default text-[12px] text-[#ffffff80]'>
      ...
      {dataItem.duration && (
        <div className='flex flex-1 flex-shrink-0 items-center justify-between text-[12px] text-[#ffffff80]'>
          {!hideAlbum && (
            <Link
              to={`${PATH.album}/${dataItem.alias}/${dataItem.album?.encodeId}`}
              className='line-clamp-1 max-w-[200px] hover:text-[#c273ed] hover:underline'
            >
